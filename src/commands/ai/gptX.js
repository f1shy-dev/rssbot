...
export const gptx = async ({
    ...
    let use16k = cmdData.aliases
        .map(i => ['16', '16k'].map(x => `${x}${i}`))
        .flat()
        .includes(mArgs._.flat()[0].split(config.prefix)[1])

    const parsedQuery = mArgs._.flat()

    // lets just make it use 16k if its longer than 4096, but enforce the ratelimits
    if (parsedQuery.length > 14440) use16k = true
    let tokenLimit = 4096
    if (use16k) {
